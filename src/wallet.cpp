...
// Digitaleurasia: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//Digitaleurasia now iterates each utxo inside of CheckStakeKernelHash()
...
// Digitaleurasia: check 'spent' consistency between wallet and txindex
// Digitaleurasia: fix wallet spent state according to txindex
...
// Digitaleurasia: disable transaction (only for coinstake)
